...
import { Logger, extendObject, writeToConsole } from "./utilities";
...
function detectProtocolForAttach(
	...
	return new Promise<{ reason?: string; protocol: string }>(
		(resolve, reject) => {
...
function detectProtocolForPidUnix(pid: number): Promise<string | null> {
	return getPidListeningOnPortUnix(INSPECTOR_PORT_DEFAULT).then<
		string | null
	>((inspectorProtocolPid) => {
