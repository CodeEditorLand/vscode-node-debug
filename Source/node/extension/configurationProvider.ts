	...
	implements vscode.DebugConfigurationProvider
{
	...
	private warnAboutUseWSL() {
		...
		if (
			vscode.workspace
				...
				.get<boolean>(SHOW_USE_WSL_IS_DEPRECATED_WARNING_SETTING, true)
		) {
			vscode.window
	...
	private async nvmSupport(config: vscode.DebugConfiguration): Promise<void> {
		...
		const { nvsFormat, remoteName, semanticVersion, arch } =
			parseVersionString(config.runtimeVersion);
