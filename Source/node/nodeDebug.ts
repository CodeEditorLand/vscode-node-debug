...
import {
	...
	V8Script,
	V8SetBreakpointArgs,
	V8SetVariableValueArgs,
	V8Simple,
} from "./nodeV8Protocol";
import * as PathUtils from "./pathUtilities";
import { ISourceMaps, SourceMap, SourceMaps } from "./sourceMaps";
...
export class Expander implements VariableContainer {
	public static SET_VALUE_ERROR = localize(
		"setVariable.error",
...
export class PropertyContainer implements VariableContainer {
	...
	public Expand(
		...
		if (filter === "named") {
			return session
				._createProperties(this._evalName, this._object, "named")
				.then((variables) => {
					if (this._this) {
						return session
							._createVariable(this._evalName, "this", this._this)
							.then((variable) => {
								if (variable) {
		...
		if (typeof start === "number" && typeof count === "number") {
			return session._createProperties(
				this._evalName,
				this._object,
				"indexed",
				start,
			...
			return session
				._createProperties(this._evalName, this._object, "all")
				.then((variables) => {
					if (this._this) {
						return session
							._createVariable(this._evalName, "this", this._this)
...
export class SetMapContainer implements VariableContainer {
	...
	public Expand(
		...
		if (filter === "named") {
			return session._createSetMapProperties(
				this._evalName,
				this._object,
			);
		}
...
export class ScopeContainer implements VariableContainer {
	...
	public Expand(
		...
		return session
			._createProperties("", this._object, filter)
			.then((variables) => {
				if (this._this) {
					return session
						._createVariable("", "this", this._this)
						.then((variable) => {
							if (variable) {
								variables.push(variable);
							}
...
export class NodeDebugSession extends LoggingDebugSession {
	...
	private static NODE_SHEBANG_MATCHER = /#! */usr/bin/env +node/;
	...
	private _processId = -1; // pid of the program launched
	private _nodeProcessId = -1; // pid of the node runtime
	...
	public constructor() {
		...
		this._node = new NodeV8Protocol((response) => {
			...
			if (response.success && response.refs) {
				...
				for (const r of response.refs) {
					this._cache(r.handle, r);
				}
				if (this._refCache.size !== oldSize) {
					this.log(
						"rc",
						`NodeV8Protocol hook: ref cache size: ${this._refCache.size}`,
					);
				}
			}
		});
	...
	private _handleNodeExceptionEvent(eventBody: V8ExceptionEventBody): void {
		...
		if (
			eventBody.sourceLineText &&
			typeof eventBody.sourceColumn === "number"
		) {
			const source = eventBody.sourceLineText.substr(
	...
	private _handleNodeBreakEvent(eventBody: V8BreakEventBody): void {
		...
		if (Array.isArray(breakpoints) && breakpoints.length > 0) {
			...
			if (!this._gotEntryEvent && id === 1) {
				...
				this.log(
					"la",
					"_handleNodeBreakEvent: suppressed stop-on-entry event",
				);
				// do not send event now
				this._rememberEntryLocation(
					eventBody.script.name,
	...
	private _sendBreakpointStoppedEvent(breakpointId: number): void {
		// evaluate hit counts
		const ibp = this._hitCounts.get(breakpointId);
		if (ibp) {
			ibp.hitCount++;
	...
	private _sendStoppedEvent(
		...
		const e = new StoppedEvent(
			reason,
			NodeDebugSession.DUMMY_THREAD_ID,
			exception_text,
		);
	...
	private isSkipped(path: string): boolean {
		return this._skipFiles
			? PathUtils.multiGlobMatches(this._skipFiles, path)
			: false;
	}
	...
	private _skip(event: V8EventBody): boolean {
		if (this._skipFiles) {
			const path = this._scriptToPath(event.script);
	...
	private _skipGenerated(event: V8EventBody): Promise<boolean> {
		...
		if (this._smartStep) {
			...
			const column = this._adjustColumn(line, event.sourceColumn);

			return this._sourceMaps
				.CannotMapLine(localPath, null, line, column)
				.then((skip) => {
					return skip;
				});
		}
	...
	private toggleSkippingResource(
		...
		if (this._sourceMaps) {
			return this._sourceMaps.AllSources(path).then((sources) => {
				if (sources && sources.length > 0) {
					(<DebugProtocol.Source>src).sources = sources.map(
	...
	private async launchRequest2(
		...
		if (
			this._supportsRunInTerminalRequest &&
			(this._console === "externalTerminal" ||
				this._console === "integratedTerminal")
		) {
			...
			this.runInTerminalRequest(
				termArgs,
				NodeDebugSession.RUNINTERMINAL_TIMEOUT,
				(runResponse) => {
			...
			envVars = PathUtils.extendObject(
				PathUtils.extendObject({}, <ProcessEnvWithNull>process.env),
				envVars,
			);
			...
			if (
				...
				wslLaunchArgs.executable.indexOf(" ") > 0
			) {
	...
	private _isRunningWithRetry(
		retryCount: number,
		completeDispatch: (value: boolean) => void,
		errorDispatch: (error: V8EvaluateResponse) => void,
	): void {
		this._node.command(
			"evaluate",
			{ expression: "process.pid", global: true },
			(resp: V8EvaluateResponse) => {
				if (resp.success && resp.body.value !== undefined) {
					...
					resp.message.indexOf("process is not defined") >= 0
				) {
					this.log(
						"la",
	...
	private logNodeVersion(): void {
		this._node.command(
			"evaluate",
			{ expression: "process.version", global: true },
			(resp: V8EvaluateResponse) => {
				if (resp.success && resp.body.value !== undefined) {
				 ...
				 */
					this.sendEvent(
						new OutputEvent("nodeVersion", "telemetry", {
							version,
						}),
					);
					this.log(
			...
			if (
				...
				try {
					const contents = FS.readFileSync(
						Path.join(__dirname, NodeDebugSession.DEBUG_INJECTION),
						"utf8",
					);
	 ...
	 */
	private _startInitialize(stopped: boolean, n = 0): void {
		if (n === 0) {
		...
		if (this._gotEntryEvent) {
			...
			this.log(
				"la",
				`_startInitialize: no entry event after ${n} retries; giving up`,
			);
	...
	protected setBreakPointsRequest(
		...
		this.log(
			"bp",
			`setBreakPointsRequest: ${JSON.stringify(
				args.source,
		...
		if (sourcePath) {
			...
			if (this._modifiedSources.has(sourcePath)) {
				...
				typeof args.sourceModified === "boolean" &&
				args.sourceModified
			) {
				keepUnverified = true;
		...
		if (source.adapterData) {
			if (source.adapterData.inlinePath) {
				...
				this._mapSourceAndUpdateBreakpoints(
					response,
		...
		if (
			sourcePath &&
			NodeDebugSession.NODE_INTERNALS_PREFIX.test(sourcePath)
		) {
			// an internal module
			this._findScript(this._pathToScript(sourcePath)).then(
				(scriptId) => {
					if (scriptId >= 0) {
	...
	private _mapSourceAndUpdateBreakpoints(
		response: DebugProtocol.SetBreakpointsResponse,
		path: string,
		lbs: InternalSourceBreakpoint[],
	): void {
		const generated = "";
		...
		Promise.resolve(generated)
			...
			.then((generated) => {
				if (
					generated !== null &&
					PathUtils.pathCompare(generated, path)
				) {
				...
				if (generated) {
					...
					Promise.all(
						lbs.map((lbrkpt) =>
							this._sourceMaps.MapFromSource(
								path,
								lbrkpt.line,
								lbrkpt.column,
							),
	...
	private _updateBreakpoints(
		...
		this._node
			...
			.then(() => {
				return Promise.all(
					lbs.map((bp) =>
						this._setBreakpoint(scriptId, path, bp, sourcemap),
					),
				);
			})
			.then((result) => {
	...
	private _setBreakpoint(
		...
		return this._node
			...
			.then((resp) => {
				...
				if (path && sourcemap) {
					if (
						actualLine !== args.line ||
						actualColumn !== args.column
					) {
	...
	private async _setBreakpoint2(
		ibp: InternalSourceBreakpoint,
		path: string | null,
		actualSrcLine: number,
		actualSrcColumn: number,
		actualLine: number,
		actualColumn: number,
	): Promise<Breakpoint> {
		...
		if (
			path &&
			PathUtils.pathCompare(this._entryPath, path) &&
			this._entryLine === actualLine &&
			this._entryColumn === actualColumn
		) {
			// only relevant if the breakpoints matches entrypoint
	...
	protected setFunctionBreakPointsRequest(
		...
		this._clearBreakpoints(this._functionBreakpoints)
			.then(() => {
				...
				return Promise.all(
					args.breakpoints.map((functionBreakpoint) =>
						this._setFunctionBreakpoint(functionBreakpoint),
					),
				);
			})
			.then((results) => {
				...
				this.log(
					"bp",
					`setFunctionBreakPointsRequest: result ${JSON.stringify(
						results,
					)}`,
				);
			})
	 ...
	 */
	private _setFunctionBreakpoint(
		functionBreakpoint: DebugProtocol.FunctionBreakpoint,
	): Promise<Breakpoint> {
		const args: V8SetBreakpointArgs = {
			type: "function",
	...
	protected setExceptionBreakPointsRequest(
		...
		this.log(
			"bp",
			`setExceptionBreakPointsRequest: ${JSON.stringify(args.filters)}`,
		);
		...
		Promise.all([
			...
			.catch((err) => {
				this.sendErrorResponse(
					response,
					2024,
					"Configuring exception break options failed ({_nodeError}).",
					{ _nodeError: err.message },
					ErrorDestination.Telemetry,
				);
			});
	}
	...
	protected stackTraceRequest(
		...
		if (threadReference !== NodeDebugSession.DUMMY_THREAD_ID) {
			this.sendErrorResponse(
				response,
				2014,
				"Unexpected thread reference {_thread}.",
				...
				if (error.message === "no stack") {
					if (this._stoppedReason === "pause") {
						...
						this.sendErrorResponse(
							response,
							2023,
							localize("VSND2023", "No call stack available."),
						);
					}
				} else {
					this.sendErrorResponse(
						...
						{ _command: error.command, _error: error.message },
					);
				}
			});
	}
	...
	private _createStackFrame(frame: V8Frame): Promise<StackFrame> {
		...
		return this._resolveValues([
			...
			const column = this._adjustColumn(line, frame.column);
				...
				if (name) {
					if (this._mapToFilesOnDisk) {
						...
						if (PathUtils.isAbsolutePath(name)) {
							...
							// if launch.json defines localRoot and remoteRoot try to convert remote path back to a local path
							const localPath = this._remoteToLocal(remotePath);
							...
							if (this._sourceMaps) {
								...
								return this._loadScript(script_val.id).then(
									(script) => {
										return this._createStackFrameFromSourceMap(
											frame,
											script.contents,
											name,
											localPath,
	 ...
	 */
	private _createStackFrameFromSourceMap(
		frame: V8Frame,
		content: string,
		name: string,
		localPath: string,
		...
		return this._sourceMaps
			.MapToSource(localPath, content, line, column)
			.then((mapresult) => {
				if (mapresult) {
					this.log(
						"sm",
						`_createStackFrameFromSourceMap: gen: '${localPath}' ${line}:${column} -> src: '${mapresult.path}' ${mapresult.line}:${mapresult.column}`,
					);

					return this._sameFile(
						mapresult.path,
						this._compareContents,
						0,
						mapresult.content,
					).then((same) => {
						if (same) {
							...
							const src = this._createSource(
								true,
								mapresult.path,
								this.convertDebuggerPathToClient(
									mapresult.path,
								),
							);
	 ...
	 */
	private _createStackFrameFromPath(
		frame: V8Frame,
		name: string,
		localPath: string,
		remotePath: string,
			...
			(same) => {
				...
				if (same) {
					...
					src = this._createSource(
						false,
						name,
						this.convertDebuggerPathToClient(localPath),
					);
				} else {
	 ...
	 */
	private _createStackFrameFromSource(
		frame: V8Frame,
				...
				if (compareContents && (script_id || content)) {
					return Promise.all([
						...
						content
							...
							: this._loadScript(script_id).then(
	...
	private static SCOPE_NAMES = [
		localize(
			{
				key: "scope.global",
				comment: ["https://github.com/Microsoft/vscode/issues/4569"],
			},
			"Global",
		),
		localize(
				...
				return Promise.all(
					scopes.map((scope) => {
						...
						if (
							...
							if (
								type === 1 &&
								typeof scopesResponse.body.vscode_locals ===
									"number"
							) {
								expensive = true;
								scopeName = localize(
									...
									"Local ({0} of {1})",
									scopesArgs.maxLocals,
									scopesResponse.body.vscode_locals,
								);
							} else {
								...
								if (x) {
									return new Scope(
										...
										this._variableHandles.create(
											new ScopeContainer(scope, x, extra),
										),
										expensive,
									);
								}
	...
	public _createProperties(
		...
		if (obj && !obj.properties) {
			...
			if (this._nodeInjectionAvailable) {
				...
				if (
					typeof obj.vscode_indexedCnt === "number" &&
	 ...
	 */
	private _createPropertyVariables(
		evalName: string | undefined,
		obj: V8Object | null,
						...
						return this._node
							...
							.catch((err) => {
								return this._createVar(
									this._getEvaluateName(evalName, name),
									name,
									"undefined",
								);
							});
	...
	public _createVariable(
		...
		*/
		...
		switch (val.type) {
			...
			case "string":
				return this._createStringVariable(
					evalName,
					name,
					val,
					doPreview
				...
				if (typeof simple.value === "boolean") {
					return Promise.resolve(
						this._createVar(
							en,
							name,
							simple.value.toString().toLowerCase(),
						),
			...
			case "error": {
				...
				switch (value) {
					...
					case "Float32Array":
					case "Float64Array":
						return this._createArrayVariable(
							evalName,
							name,
							val,
							doPreview,
					...
					case "Object":
						return this._resolveValues(
							...
							if (
								...
								if (object.status) {
									// promises and generators have a status attribute
									value += ` { ${object.status} }`;
								}
							} else if (object.properties) {
								return this._objectPreview(
									object,
									doPreview,
								).then((preview) => {
									if (preview) {
		...
		return Promise.resolve(
			this._createVar(
				en,
				name,
				simple.value ? simple.value.toString() : "undefined",
			),
		);
	}
	...
	private _objectPreview(
		...
		if (
			...
			return this._createPropertyVariables(
				undefined,
				object,
				object.properties.slice(0, NodeDebugSession.PREVIEW_PROPERTIES),
				false,
			).then((props) => {
				let preview = "{";
				for (let i = 0; i < props.length; i++) {
					...
					if (i < props.length - 1) {
						preview += ", ";
					} else if (propcnt > NodeDebugSession.PREVIEW_PROPERTIES) {
						preview += " …";
					}
				}
				preview += "}";
		...
		if (doPreview && array && array.properties && length > 0) {
			...
			return this._createPropertyVariables(
				...
				for (let i = 0; i < props.length; i++) {
					...
					if (i < props.length - 1) {
						preview += ", ";
					} else if (length > NodeDebugSession.PREVIEW_PROPERTIES) {
						preview += " …";
					}
				}
				preview += "]";
			...
			return this._arrayPreview(array, indexedSize, doPreview).then(
				(preview) => {
					...
					return this._createVar(
						...
						v,
						this._variableHandles.create(
							new PropertyContainer(en, array),
						),
						indexedSize,
						namedSize,
					);
	...
	private _getArraySize(array: V8Object): Promise<number[]> {
		if (
			typeof array.vscode_indexedCnt === "number" &&
			typeof array.vscode_namedCnt === "number"
		) {
			return Promise.resolve([
				array.vscode_indexedCnt,
	...
	private _createSetMapVariable(
		...
		return this._node.evaluate(args).then((response) => {
			...
			return this._createVar(
				en,
				name,
				`${typename}[${indexedSize}]`,
				this._variableHandles.create(new SetMapContainer(en, obj)),
				indexedSize,
				namedSize,
			);
		});
	...
	public _createSetMapProperties(
		evalName: string | undefined,
		obj: V8Handle,
	): Promise<Variable[]> {
		const args = {
	...
	public _createSetElements(
		set: V8Handle,
		start: number,
		count: number,
	): Promise<Variable[]> {
		const args = {
			expression: `var r = [], i = 0; set.forEach(v => { if (i >= ${start} && i < ${
				start + count
			}) r.push(v); i++; }); r`,
			disable_break: true,
			additional_context: [{ name: "set", handle: set.handle }],
		};
	...
	public _createMapElements(
		...
		const args = {
			expression: `var r=[],i=0; map.forEach((v,k) => { if (i >= ${start} && i < ${
				start + count
			}) { r.push(k+' → '+v); r.push(k); r.push(v);} i++; }); r`,
			disable_break: true,
			additional_context: [{ name: "map", handle: map.handle }],
		};
				...
				for (let i = 0; i < selectedProperties.length; i += 3) {
					...
					variables.push(
						this._createVar(
							...
							(start + i / 3).toString(),
							<string>x.value,
							this._variableHandles.create(expander),
						),
					);
				}
				return variables;
		...
		if (
			...
			return this._node.evaluate(args).then((response) => {
				...
				return this._createVar(
					en,
					name,
					this._escapeStringValue(str_val),
				);
			});
		} else {
			return Promise.resolve(
	...
	public _setPropertyValue(
		...
		if (this._node.v8Version) {
			...
			const args = {
				...
				disable_break: true,
				additional_context: [{ name: "obj", handle: objHandle }],
				maxStringLength: NodeDebugSession.MAX_STRING_LENGTH,
			};
	...
	protected evaluateRequest(
		...
		this._node.command(
			this._nodeInjectionAvailable ? "vscode_evaluate" : "evaluate",
			evalArgs,
			(resp: V8EvaluateResponse) => {
				if (resp.success) {
					this._createVariable(undefined, "evaluate", resp.body).then(
						(v) => {
							if (v) {
								response.body = {
									result: v.value,
	...
	private sourceRequest2(
		...
		if (srcSource) {
			...
			if (srcSource.scriptId) {
				// load script content
				this._loadScript(srcSource.scriptId)
					.then((script) => {
	...
	private _loadScript(scriptIdOrPath: number | string): Promise<Script> {
		if (typeof scriptIdOrPath === "number") {
			...
			return this._node.scripts(args).then((nodeResponse) => {
				for (const result of nodeResponse.body) {
	...
	protected completionsRequest(
		...
		if (dot >= 0) {
			...
			if (
				rest.length === 0 ||
				NodeDebugSession.PROPERTY_NAME_MATCHER.test(rest)
			) {
				// empty or proper attribute name
		...
		if (expression) {
			...
			this._node
				...
				.then((resp) => {
					...
					const arrays = <string[]>(
						JSON.parse(<string>resp.body.value)
					);
	...
	protected exceptionInfoRequest(
		...
		if (args.threadId !== NodeDebugSession.DUMMY_THREAD_ID) {
			this.sendErrorResponse(
				response,
				2030,
				"exceptionInfoRequest error: invalid thread {_thread}.",
		...
		if (this._exception) {
			...
			Promise.resolve(this._exception.exception)
				.then((exception) => {
					if (exception) {
			...
			this.sendErrorResponse(
				response,
				2032,
				"exceptionInfoRequest error: no stored exception",
				undefined,
				ErrorDestination.Telemetry,
			);
		}
	...
	public log(traceCategory: string, message: string) {
		if (
			this._trace &&
			(this._traceAll || this._trace.indexOf(traceCategory) >= 0)
		) {
	 ...
	 */
	private sendNotExistErrorResponse(
		response: DebugProtocol.Response,
		attribute: string,
		path: string,
	) {
	 ...
	 */
	private sendRelativePathErrorResponse(
		response: DebugProtocol.Response,
		attribute: string,
		path: string,
	) {
	 ...
	 */
	private sendErrorResponseWithInfoLink(
		response: DebugProtocol.Response,
		code: number,
		format: string,
		variables: any,
	...
	private _localToRemote(localPath: string): string {
		if (this._remoteRoot && this._localRoot) {
			const relPath = PathUtils.makeRelative2(this._localRoot, localPath);
	...
	private _remoteToLocal(remotePath: string): string {
		if (this._remoteRoot && this._localRoot) {
			const relPath = PathUtils.makeRelative2(
				this._remoteRoot,
				remotePath,
			);
	...
	private _sendNodeResponse(
		response: DebugProtocol.Response,
		nodeResponse: NodeV8Response,
	): void {
		if (nodeResponse.success) {
			this.sendResponse(response);
		} else {
			...
			if (errmsg.indexOf("unresponsive") >= 0) {
				...
				this.sendErrorResponse(
					response,
					2013,
					"Node.js request '{_request}' failed (reason: {_error}).",
					{ _request: nodeResponse.command, _error: errmsg },
					ErrorDestination.Telemetry,
				);
			}
		}
	...
	private _resolveToCache(
		handles: number[],
	): Promise<(V8Object | undefined)[]> {
		...
		if (lookup.length > 0) {
			...
			return Promise.resolve(
				handles.map((handle) => this._refCache.get(handle)),
			);
		}
	}

	private _rememberEntryLocation(
		path: string,
		line: number,
		column: number,
	): void {
	...
	private static isJavaScript(path: string): boolean {
		...
		} else if (Path.basename(path).toLowerCase() === "www") {
			return true;
		}
