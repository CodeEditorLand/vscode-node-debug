			...
			if (semVerString) {
				...
				if (match && match.length === 4 && parseInt(match[1]) >= 8) {
					...
					if (process.platform === "win32") {
						env.Path =
							env.LEGACY_NODE_PATH ||
							`${env.NVM_HOME}\\v7.9.0;${env.Path}`;
					} else {
						env.PATH =
							env.LEGACY_NODE_PATH ||
							`${env.NVM_DIR}/versions/node/v7.9.0/bin:${env.PATH}`;
					}
				}
			}
		}

		dc = new DebugClient("node", DEBUG_ADAPTER, "legacy-node", {
			env: env,
		});
