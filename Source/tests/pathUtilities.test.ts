...
suite("pathUtilities", () => {
	...
	suite("makeRelative(target, path)", () => {
		test("identical paths should return empty string", () => {
			if (process.platform === "win32") {
				assert.equal(
					PathUtils.makeRelative("c:\\a\\b", "c:\\a\\b"),
					""
				);
			} else {
		...
		test("target and path same length", () => {
			if (process.platform === "win32") {
				...
				assert.equal(
					PathUtils.makeRelative("/a/b/c/d/e/f", "/a/b/c/g/h/j"),
					"g/h/j"
				);
			}
		});
		...
		test("target is longer", () => {
			if (process.platform === "win32") {
				assert.equal(
					PathUtils.makeRelative("c:\\a\\b\\c\\d", "c:\\a\\b\\c"),
					""
				);
			} else {
		...
		test("path is longer", () => {
			if (process.platform === "win32") {
				...
				assert.equal(
					PathUtils.makeRelative("/a/b/c/d", "/a/b/c/d/e"),
					"e"
				);
			}
		});
	...
	suite("makeRelative2(from, to)", () => {
		...
		test("from and to same length", () => {
			assert.equal(
				PathUtils.makeRelative2("/a/b/c/d/e/f", "/a/b/c/g/h/j"),
				"../../g/h/j"
			);
		});
		...
		test("from is longer", () => {
			...
			assert.equal(
				PathUtils.makeRelative2("/a/b/c/d/e", "/a/d/e"),
				"../../d/e"
			);
		});
		...
		test("to is longer", () => {
			assert.equal(
				PathUtils.makeRelative2("/a/b/c/d", "/a/b/c/d/e"),
				"e"
			);
			assert.equal(
				PathUtils.makeRelative2("/a/b/c/d", "/a/b/c/d/e/f"),
				"e/f"
			);
	...
	suite("normalizeDriveLetter", () => {
		test("should return an unmodified path if there is no drive letter", () => {
			assert.equal(
				PathUtils.normalizeDriveLetter("/a/b/c/d"),
				"/a/b/c/d"
			);
			assert.equal(
				PathUtils.normalizeDriveLetter("C/hello/world"),
				"C/hello/world"
			);
		});
	...
	suite("multiGlob", () => {
		...
		test("one pattern up", () => {
			return PathUtils.multiGlob(["testdata/glob/f1/../f2/**/*"]).then(
				(paths) => {
		...
		test("one pattern with curly brackets, one exclude", () => {
			return PathUtils.multiGlob([
				"testdata/glob/{f1,f2,f3}/**/*",
				"!testdata/glob/f2/**",
			]).then((paths) => {
				assert.equal(paths.length, 4);
			});
	...
	suite("multiGlobMatches", () => {
		test("simple", () => {
			...
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/foo/abc.js"),
				true
			);
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/foo/abc.ts"),
				false
			);
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/foo/bar/xyz/abc.js"),
				true
			);
		});
		...
		test("braces", () => {
			...
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/bar/abc.js"),
				true
			);
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/xyz/abc.js"),
				false
			);
		});
		...
		test("include/exclude", () => {
			const patterns = ["**/*.js", "!/foo/**/bar.js"];
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/foo/xyz.js"),
				true
			);
			assert.equal(
				PathUtils.multiGlobMatches(patterns, "/foo/bar.js"),
				false
			);
		});
	});
	...
	suite("extendObject", () => {
		test("extend", () => {
			assert.deepEqual(
				PathUtils.extendObject<any>({ foo: "bar" }, { abc: 123 }),
				{ foo: "bar", abc: 123 }
			);
		});
		...
		test("override", () => {
			assert.deepEqual(
				PathUtils.extendObject<any>({ foo: "bar" }, { foo: 123 }),
				{ foo: 123 }
			);
		});
